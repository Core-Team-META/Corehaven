--[[
    ...
    API.SlowTextPrint(UIText,string,number,[audio],[bool])
    API.LerpAlphaChildren(CoreObject,number,number)
...
function API.LerpAlphaChildren(ui,alpha,timeTilEach)
    ...
    if(ui:IsA("UIImage") or ui:IsA("UIText")) then
        ...
        newColor.a = alpha
        Task.Spawn(function() colorTo(ui,ui:GetColor(),newColor,timeTilEach)end,0)
    end
